...
namespace vush {
  ...
  //
  struct Downcast_Tag {
    explicit Downcast_Tag() =
      default; // Explicit constructor so that it may not be constructed via {}
  };
  ...
  struct Deleter {
    ...
    Deleter(Allocator* allocator, i64 size, i64 alignment)
      : size(size), alignment(alignment), allocator(allocator)
    {
    }
  ...
  public:
    ...
    explicit Owning_Ptr(T* ptr, Allocator* allocator)
      : _pointer(ptr), _deleter(allocator, sizeof(T), alignof(T))
    {
    }

    template<typename U>
    explicit Owning_Ptr(Downcast_Tag, U* ptr, Allocator* allocator)
      : _pointer(static_cast<T*>(ptr)), _deleter(allocator, sizeof(T), alignof(T))
    {
    }

    template<typename U>
    explicit Owning_Ptr(Downcast_Tag, Owning_Ptr<U>&& other)
      : _pointer(static_cast<T*>(other.release())), _deleter(other._deleter)
    {
    }
    ...
    template<typename U>
    Owning_Ptr(Owning_Ptr<U>&& other,
               anton::enable_if<anton::is_convertible<U*, T*>, anton::i32> = 0)

...
namespace vush {
    ...
    namespace spirv {
        ...
        [[nodiscard]] static anton::Expected<anton::String, anton::String> resolve_import_path(Allocator& allocator, anton::String_View const source_name,
                ...
                if(exists) {
                    if(!found) {
                        ...
                        return {anton::expected_error,
                                anton::format(&allocator, u8"ambiguous source '{}' matches '{}' and '{}'", source_name, out_path, resolved_path)};
                    }
                }
            }
            ...
            if(found) {
                ...
                return {anton::expected_error, anton::format(&allocator, u8"could not find the source file '{}'", source_name)};
            }
        }

        [[nodiscard]] static anton::Expected<Source_Import_Result, anton::String> file_read_callback(Allocator& allocator, anton::String_View const source_name,
                                                                                                     void* user_data) {
            anton::Slice<anton::String const> const& import_directories = *(anton::Slice<anton::String const> const*)user_data;
            anton::Expected<anton::String, anton::String> res = resolve_import_path(allocator, source_name, import_directories);
            if(!res) {
            ...
            if(!file.open(path)) {
                return {anton::expected_error, anton::format(&allocator, u8"could not open '{}' for reading", path)};
            }
            ...
            return {anton::expected_value, Source_Import_Result{ANTON_MOV(res.value()), ANTON_MOV(file_contents)}};
        }

...
namespace vush {
    ...
    //
    [[nodiscard]] static anton::Expected<void, Error> check_and_add_symbol(Context& ctx, Scoped_Symbol_Table& symtable, Symbol const& symbol) {
    ...
    [[nodiscard]] static anton::Expected<void, Error> defcheck_expression(Context& ctx, Scoped_Symbol_Table& symtable, ast::Expr const* const expression) {
        switch(expression->node_kind) {
            ...
            case ast::Node_Kind::expr_if: {
                ...
                anton::Expected<void, Error> result_condition = defcheck_expression(ctx, symtable, node->condition);
                if(!result_condition) {
                    return ANTON_MOV(result_condition);
                }

                anton::Expected<void, Error> result_then = defcheck_expression(ctx, symtable, node->then_branch);
                if(!result_then) {
                    return ANTON_MOV(result_then);
                }

                anton::Expected<void, Error> result_else = defcheck_expression(ctx, symtable, node->else_branch);
                if(!result_else) {
    ...
    [[nodiscard]] static anton::Expected<void, Error> defcheck_statements(Context& ctx, Scoped_Symbol_Table& symtable, ast::Node_List const statements) {
        ...
        for(ast::Node const* const stmt: statements) {
            switch(stmt->node_kind) {
                case ast::Node_Kind::variable: {
                    ...
                    anton::Expected<void, Error> symbol_res = check_and_add_symbol(ctx, symtable, Symbol(node->identifier->value, node));
                    if(!symbol_res) {
                ...
                case ast::Node_Kind::stmt_if: {
                    ...
                    if(anton::Expected<void, Error> res = defcheck_expression(ctx, symtable, node->condition); !res) {
                        return ANTON_MOV(res);
                    }

                    if(anton::Expected<void, Error> res = defcheck_statements(ctx, symtable, node->then_branch); !res) {
                        return ANTON_MOV(res);
                    }
                ...
                case ast::Node_Kind::stmt_loop: {
                    ...
                    anton::Expected<void, Error> continuation_result = defcheck_statements(ctx, symtable, node->continuation);
                    if(!continuation_result) {
                        return ANTON_MOV(continuation_result);
                    }

                    anton::Expected<void, Error> statements_result = defcheck_statements(ctx, symtable, node->statements);
                ...
                case ast::Node_Kind::stmt_switch: {
                    ...
                    anton::Expected<void, Error> switch_expr_result = defcheck_expression(ctx, symtable, node->expression);
    ...
    [[nodiscard]] static anton::Expected<void, Error> defcheck_function(Context& ctx, Scoped_Symbol_Table& symtable, ast::Decl_Function const* const fn) {
        ...
        for(ast::Func_Parameter const* const parameter: fn->parameters) {
            ...
            anton::Expected<void, Error> symbol_result = check_and_add_symbol(ctx, symtable, Symbol(parameter->identifier->value, parameter));
            if(!symbol_result) {
                return ANTON_MOV(symbol_result);
            }
        }

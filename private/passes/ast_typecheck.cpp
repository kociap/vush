...
namespace vush {
    // select_overload
    //
    [[nodiscard]] static anton::Expected<ast::Decl_Function const*, Error> select_overload(Context& ctx, ast::Expr_Call const* const call,
                                                                                           ast::Decl_Overloaded_Function const* const overloads);

    // evaluate_expression_type
    // Evaluate the type of an expression node. Recursively descend the
    // hierarchy evaluating types of all expressions in the tree. To prevent
    // costly reevaluation, evaluated types are maintained in a cache within
    // Context.
    //
    [[nodiscard]] static anton::Expected<ast::Type const*, Error> evaluate_expression_type(Context& ctx, ast::Node const* const node);

    anton::Expected<ast::Decl_Function const*, Error> select_overload(Context& ctx, ast::Expr_Call const* const call,
                                                                      ast::Decl_Overloaded_Function const* const overloads) {
        ...
        for(ast::Decl_Function const* const fn: overloads->overloads) {
            ...
            for(auto const [argument, parameter]: anton::Range(ANTON_MOV(begin), ANTON_MOV(end))) {
                anton::Expected<ast::Type const*, Error> eval_result = evaluate_expression_type(ctx, argument);
                if(!eval_result) {
    ...
    anton::Expected<ast::Type const*, Error> evaluate_expression_type(Context& ctx, ast::Node const* const node) {
        ...
        switch(node->node_kind) {
            ...
            case ast::Node_Kind::expr_identifier: {
                ...
                switch(definition->node_kind) {
                    ...
                    case ast::Node_Kind::func_parameter: {
                        ast::Func_Parameter const* const parameter = static_cast<ast::Func_Parameter const*>(definition);
                    ...
                    case ast::Node_Kind::decl_overloaded_function: {
                        ...
                        return {anton::expected_error, err_identifier_names_a_function_but_is_not_called(ctx, node->source_info)};
                    }
            ...
            case ast::Node_Kind::expr_call: {
                ...
                ANTON_ASSERT(definition->node_kind == ast::Node_Kind::decl_overloaded_function, "expr_call's definition is not a decl_overloaded_function");
                ast::Decl_Overloaded_Function const* const fn = static_cast<ast::Decl_Overloaded_Function const*>(definition);
            ...
            case ast::Node_Kind::expr_init: {
                ...
                switch(generic_type->node_kind) {
                    case ast::Node_Kind::type_builtin: {
                        // Initialization of builtin types has already been validated, hence we proceed with vectors.
                        // TODO: We ought to make swizzles an exception to member lookup since generating all
                        //       permutations would result in considerably too many members. Additionally, we would have
                        //       to include an exception for their initialization.
                    ...
                    case ast::Node_Kind::type_user_defined: {
                        ast::Type_User_Defined const* const type = static_cast<ast::Type_User_Defined const*>(generic_type);
                        ast::Node const* const definition_node = ctx.find_type_definition(type->value);
                        ANTON_ASSERT(definition_node->node_kind == ast::Node_Kind::decl_struct, "type definition node is not a decl_struct");
                        ...
                        for(ast::Initializer const* const generic_initializer: expr->initializers) {
                            ANTON_ASSERT(generic_initializer->node_kind == ast::Node_Kind::named_initializer, "struct initializer is not named_initializer");
                            ...
                            ast::Member_List::iterator const member =
                                anton::find_if(members_begin, members_end,
                                               [identifier](ast::Struct_Member const* const member) { return member->identifier->value == identifier; });
                            if(member == members_end) {
                                return {anton::expected_error, err_named_initializer_no_field_named(ctx, decl, initializer)};
                            }

                            anton::Expected<ast::Type const*, Error> result = evaluate_expression_type(ctx, initializer->expression);
                            if(!result) {
                                return ANTON_MOV(result);
                            }

                            if(!compare_types_equal(*(*member)->type, *result.value())) {
                                return {anton::expected_error, err_cannot_convert_type(ctx, (*member)->type, result.value())};
                            }
                        }
                    } break;
            ...
            case ast::Node_Kind::expr_assignment: {
                ...
                anton::Expected<ast::Type const*, Error> result_lhs = evaluate_expression_type(ctx, expr->lhs);
                if(!result_lhs) {
                    return ANTON_MOV(result_lhs);
                }

                anton::Expected<ast::Type const*, Error> result_rhs = evaluate_expression_type(ctx, expr->rhs);
                if(!result_rhs) {
            ...
            case ast::Node_Kind::expr_parentheses: {
                ...
                anton::Expected<ast::Type const*, Error> result = evaluate_expression_type(ctx, expr->expression);
                if(result) {
            ...
            case ast::Node_Kind::expr_if: {
                ...
                anton::Expected<ast::Type const*, Error> condition_result = evaluate_expression_type(ctx, expr->condition);
                if(!condition_result) {
                ...
                if(!compare_types_equal(*bool_type, *condition_type)) {
                    return {anton::expected_error, err_condition_not_of_bool_type(ctx, expr->condition, condition_type)};
                }

                anton::Expected<ast::Type const*, Error> then_result = evaluate_expression_type(ctx, expr->then_branch);
                if(!then_result) {
                    return ANTON_MOV(then_result);
                }

                anton::Expected<ast::Type const*, Error> else_result = evaluate_expression_type(ctx, expr->else_branch);
                if(!else_result) {
                ...
                if(!compare_types_equal(*then_type, *else_type)) {
                    return {anton::expected_error, err_incompatible_if_expression_types(ctx, then_type, expr->then_branch, else_type, expr->else_branch)};
                }
            ...
            case ast::Node_Kind::expr_array_access: {
                ...
                anton::Expected<ast::Type const*, Error> base_result = evaluate_expression_type(ctx, expr->base);
                if(!base_result) {
                ...
                anton::Expected<ast::Type const*, Error> index_result = evaluate_expression_type(ctx, expr->index);
                if(!index_result) {
                    return ANTON_MOV(index_result);
                }

                if(!is_integer(*index_result.value())) {
                    return {anton::expected_error, err_array_index_is_not_integer(ctx, index_result.value(), expr->index)};
                }
            ...
            case ast::Node_Kind::expr_member_access: {
                ast::Expr_Member_Access const* const expr = static_cast<ast::Expr_Member_Access const*>(node);
                ...
                switch(generic_type->node_kind) {
                    case ast::Node_Kind::type_builtin: {
                        ...
                        return {anton::expected_error, err_type_has_no_member_named(ctx, generic_type, expr->member)};
                    } break;
    ...
    [[nodiscard]] static anton::Expected<void, Error> typecheck_statements(Context& ctx, ast::Node_List const statements) {
        for(ast::Node const* const stmt: statements) {
            switch(stmt->node_kind) {
                case ast::Node_Kind::variable: {
                    ast::Variable const* const node = static_cast<ast::Variable const*>(stmt);
                    if(node->initializer != nullptr) {
                        anton::Expected<ast::Type const*, Error> result = evaluate_expression_type(ctx, node->initializer);
                        if(!result) {
                            return {anton::expected_error, ANTON_MOV(result.error())};
                        }
                ...
                case ast::Node_Kind::stmt_if: {
                    ...
                    anton::Expected<ast::Type const*, Error> condition_result = evaluate_expression_type(ctx, node->condition);
                    if(!condition_result) {
                    ...
                    if(!compare_types_equal(*condition_type, *bool_type)) {
                        return {anton::expected_error, err_condition_not_of_bool_type(ctx, node->condition, condition_type)};
                    }

                    if(anton::Expected<void, Error> result = typecheck_statements(ctx, node->then_branch); !result) {
                        return ANTON_MOV(result);
                    }

                    if(anton::Expected<void, Error> result = typecheck_statements(ctx, node->else_branch); !result) {
                        return ANTON_MOV(result);
                    }
                } break;
                ...
                case ast::Node_Kind::stmt_loop: {
                    ...
                    anton::Expected<void, Error> continuation_result = typecheck_statements(ctx, node->continuation);
                    if(!continuation_result) {
                        return ANTON_MOV(continuation_result);
                    }

                    anton::Expected<void, Error> statements_result = typecheck_statements(ctx, node->statements);
                ...
                case ast::Node_Kind::stmt_switch: {
                    ...
                    anton::Expected<ast::Type const*, Error> expression_result = evaluate_expression_type(ctx, node->expression);
                    ...
                    if(!compare_types_equal(*expression_type, *i32_type) && !compare_types_equal(*expression_type, *u32_type)) {
                        return {anton::expected_error, err_condition_not_of_bool_type(ctx, node->expression, expression_type)};
                    }

                    for(ast::Switch_Arm const* const arm: node->arms) {
                        anton::Expected<void, Error> statements_result = typecheck_statements(ctx, arm->statements);
                        if(!statements_result) {
                            return ANTON_MOV(statements_result);
                        }
                    }
    ...
    anton::Expected<void, Error> run_ast_typecheck_pass(Context& ctx, ast::Node_List const ast) {
        for(ast::Node const* const node: ast) {
            switch(node->node_kind) {
                case ast::Node_Kind::decl_overloaded_function: {
                    ast::Decl_Overloaded_Function const* const fn = static_cast<ast::Decl_Overloaded_Function const*>(node);
                ...
                case ast::Node_Kind::decl_stage_function: {
                    ast::Decl_Stage_Function const* const fn = static_cast<ast::Decl_Stage_Function const*>(node);

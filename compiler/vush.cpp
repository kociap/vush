...
namespace vush {
    ...
    anton::Slice<anton::String const> const import_directories)
  {
        ...
        if(!found) {
          ...
          return {
            ...
            anton::format(&allocator,
                          "ambiguous source '{}' matches '{}' and '{}'"_sv,
    ...
    if(found) {
      ...
      return {anton::expected_error,
              anton::format(&allocator,
                            "could not find the source file '{}'"_sv,
                            source_name)};
    }
  }

  [[nodiscard]] static anton::Expected<Source_Import_Result, anton::String>
  file_read_callback(Allocator& allocator, anton::String_View const source_name,
                     void* user_data)
  {
    anton::String path;
    if(begins_with(source_name, "/"_sv)) {
      // Absolute path. Keep as is.
      path = anton::String(source_name, &allocator);
    } else {
      anton::Slice<anton::String const> const& import_directories =
    ...
    return {anton::expected_value,
            Source_Import_Result{ANTON_MOV(path), ANTON_MOV(file_contents)}};
  }
  ...
  compile_to_spirv(Configuration const& config, Allocator& allocator,
                   anton::Slice<anton::String const> const& import_directories)
  {
    return compile_to_spirv(config, allocator, file_read_callback,

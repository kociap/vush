...
namespace vush {
  ...
  //
  // Evaluate the type of an expression node. Recursively descend the hierarchy
  // evaluating types of all expressions in the tree. To prevent costly
  // reevaluation, evaluated types are maintained in a cache within Context.
  //
                              ...
                              ast::Initializer const& generic_initializer)
  {
    ...
    ANTON_ASSERT(generic_initializer.node_kind ==
                   ast::Node_Kind::field_initializer,
                 "vector initializer is not a field initializer");
    // Match the type of the swizzle initializer against the type of the
    // expression.
    auto const initializer =
      static_cast<ast::Field_Initializer const&>(generic_initializer);
    auto initializer_result =
  ...
  anton::Expected<ast::Type const*, Error>
  evaluate_vector_field(Context const& ctx, ast::Type_Builtin const& type,
                        ast::Identifier const& field)
  {
    ANTON_ASSERT(is_vector(type), "type is not vector");
                              ...
                              ast::Initializer const& generic_initializer)
  {
    ...
    ANTON_ASSERT(
      generic_initializer.node_kind == ast::Node_Kind::field_initializer ||
    ...
    case ast::Node_Kind::index_initializer: {
      return {anton::expected_error,
              err_unimplemented(ctx, generic_initializer.source_info, __FILE__,
                                __LINE__)};
    }
    ...
    for(ast::Decl_Function* const fn: group->overloads) {
      ...
      for(auto const [argument, parameter]:
        ...
        anton::Optional<i64> rank_result =
          rank_conversion(parameter->type, type);
        if(!rank_result) {
          viable = false;
          break;
        }
  ...
  anton::Expected<ast::Type const*, Error>
  evaluate_expression_type(Context& ctx, ast::Expr* const generic_expr)
  {
    ...
    case ast::Node_Kind::lt_integer: {
      ast::Lt_Integer const* const lt =
        static_cast<ast::Lt_Integer const*>(generic_expr);
      switch(lt->kind) {
    ...
    case ast::Node_Kind::lt_float: {
      ast::Lt_Float const* const lt =
        static_cast<ast::Lt_Float const*>(generic_expr);
      switch(lt->kind) {
    ...
    case ast::Node_Kind::expr_identifier: {
      ...
      case ast::Node_Kind::variable: {
        ast::Variable const* const variable =
          static_cast<ast::Variable const*>(definition);
    ...
    case ast::Node_Kind::expr_init: {
      ast::Expr_Init const* const expr =
        static_cast<ast::Expr_Init const*>(generic_expr);
      ...
      case ast::Type_Kind::type_struct: {
        ...
        for(ast::Initializer const* const generic_initializer:
          ...
          if(field == fields_end) {
            return {anton::expected_error, err_field_initializer_no_field_named(
                                             ctx, decl, initializer)};
          }
    ...
    case ast::Node_Kind::expr_if: {
      ast::Expr_If const* const expr =
        static_cast<ast::Expr_If const*>(generic_expr);
    ...
    case ast::Node_Kind::expr_index: {
      ast::Expr_Index const* const expr =
        static_cast<ast::Expr_Index const*>(generic_expr);
      ...
      if(!is_array(*generic_base_type)) {
        return {anton::expected_error, err_expression_is_not_indexable(
                                         ctx, generic_base_type, expr->base)};
      }
      ...
      if(!is_integer(*index_result.value())) {
        return {anton::expected_error,
                err_array_index_is_not_integer(ctx, index_result.value(),
                                               expr->index)};
      }

      auto const base_type =
        static_cast<ast::Type_Array const*>(generic_base_type);
    ...
    case ast::Node_Kind::expr_field: {
      ast::Expr_Field const* const expr =
        static_cast<ast::Expr_Field const*>(generic_expr);
      anton::Expected<ast::Type const*, Error> result =
        evaluate_expression_type(ctx, expr->base);
      if(!result) {
        return ANTON_MOV(result);
    ...
    for(ast::Node const* const stmt: statements) {
      ...
      case ast::Node_Kind::variable: {
        ...
        if(node->initializer != nullptr) {
          ...
          if(immutable) {
            return {anton::expected_error,
                    err_immutable_variable_missing_initializer(
                      ctx, node->source_info)};
          }
        }
      } break;
      ...
      case ast::Node_Kind::stmt_if: {
        ...
        if(!is_convertible(bool_type, condition_type)) {
          return {anton::expected_error,
                  err_condition_not_of_bool_type(ctx, node->condition,
                                                 condition_type)};
        }

        if(anton::Expected<void, Error> result =
             typecheck_statements(ctx, node->then_branch);
           !result) {
          return ANTON_MOV(result);
        }

        if(anton::Expected<void, Error> result =
             typecheck_statements(ctx, node->else_branch);
           !result) {
          return ANTON_MOV(result);
        }
      } break;
      ...
      case ast::Node_Kind::stmt_switch: {
        ...
        ast::Type const* const i32_type =
          get_builtin_type(ast::Type_Builtin_Kind::e_int);
        ast::Type const* const u32_type =
          get_builtin_type(ast::Type_Builtin_Kind::e_uint);
        if(!compare_types_equal(*expression_type, *i32_type) &&
           !compare_types_equal(*expression_type, *u32_type)) {
          return {anton::expected_error,
                  err_condition_not_of_bool_type(ctx, node->expression,
                                                 expression_type)};
        }
      ...
      case ast::Node_Kind::stmt_return: {
        ...
        // TODO: Check the type of the expression being returned against the
  ...
  anton::Expected<void, Error> run_ast_typecheck_pass(Context& ctx,
                                                      ast::Node_List const ast)
  {
    for(ast::Node* const node: ast) {
      ...
      case ast::Node_Kind::decl_function: {
        ...
        anton::Expected<void, Error> result =
          typecheck_statements(ctx, fn->body);
        if(!result) {
          return ANTON_MOV(result);
        }
      } break;

...
namespace vush {
  static void
  add_overload(Allocator* const allocator,
               anton::Flat_Hash_Map<anton::String_View, Array<ast::Decl_Function*>*>& dictionary,
               ast::Decl_Function* const fn)
  {
    auto iter = dictionary.find(fn->identifier.value);
    if(iter == dictionary.end()) {
      auto const functions =
        allocate<Array<ast::Decl_Function*>>(allocator, allocator, anton::variadic_construct, fn);
  ...
  [[nodiscard]] static Array<ast::Decl_Overloaded_Function*>
  construct_overloaded_fns(Context& ctx, ast::Node_List const ast)
  {
    anton::Flat_Hash_Map<anton::String_View, Array<ast::Decl_Function*>*> overloads_dictionary(
      ctx.allocator);
    // First add builtin functions so that they are reported in diagnostics as the functions defined
    // first.
    for(ast::Decl_Overloaded_Function* const ofn:
        get_builtin_functions_declarations(ctx.allocator)) {
      for(ast::Decl_Function* const fn: ofn->overloads) {
    ...
    for(ast::Node* const decl: ast) {
      ...
      ast::Decl_Function* const fn = static_cast<ast::Decl_Function*>(decl);
      add_overload(ctx.allocator, overloads_dictionary, fn);
    }

    Array<ast::Decl_Overloaded_Function*> overloaded_fns;
  ...
  {
    Array<ast::Decl_Overloaded_Function*> const overloaded_fns = construct_overloaded_fns(ctx, ast);
    // Merge ast and overloaded functions while removing function declarations.
    Array<ast::Node*>* const result_ast = allocate<Array<ast::Node*>>(ctx.allocator, ctx.allocator);
    for(ast::Node* const node: overloaded_fns) {
